...
public class AspiURLInfo
	...
	public void launch() throws Exception
		...
		try
			{
			System.out.println("Début de récupération des info Device");
	...
	public void loadAdresse() throws IndexOutOfBoundsException
		...
		try
			...
			for(int i=0; i<infoDeviceType.values().length; i++)
				...
				listParams.add(infoDeviceType.values()[i].name());
				myDevice.setInfo(infoDeviceType.values()[i],xMLGear.getResultList(ObjetXMLReponse, listParams).get(0));
				}
			}
		catch(Exception exc)
			{
